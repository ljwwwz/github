	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				//files = (urls[i].substring(0,urls[i].lastIndexOf("/")+1)+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))) + "|";
				/* if("${input}" == "fj"){
					files = (urls[i].substring(0,urls[i].lastIndexOf("/")+1)+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1))) + "|";
				} */
				$("#${input}").val(urls[i].substring(0,urls[i].lastIndexOf("/")+1)+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1)));
				/* if("${input}" == "fj"){
					$("#${input}").val(urls[i].substring(0,urls[i].lastIndexOf("/")+1)+decodeURIComponent(urls[i].substring(urls[i].lastIndexOf("/")+1)));
				} */
			}
